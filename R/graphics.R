...
#' @param add_test Logical indicating if a statistical test should be added to the table.
#'
...
ody_add_tbl_violindotbox <- function(p, n_dec = 1, add_test = FALSE) {
  ...
  desc_tbl <-
    ...
    dplyr::mutate(
      dplyr::across(
        c("min", "q1", "median", "q3", "max", "mean", "sd"), ~round(., n_dec)
      ),
      N = as.character(.data[["N"]]),
      `Mean (SD)` = stringr::str_c(.data[["mean"]], " (", .data[["sd"]], ")"),
      `Median (IQR)` = stringr::str_c(
        .data[["median"]], " (", .data[["q1"]], ", ", .data[["q3"]], ")"
      ),
      `Min - Max` = stringr::str_c(.data[["min"]], ", ", .data[["max"]])
    ) |>
    dplyr::select(
      tidyselect::all_of(x), "N", "Mean (SD)", "Median (IQR)", "Min - Max"
    ) |>
    tidyr::pivot_longer(
